        ...
        {navItems(t).map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className={`relative flex items-center gap-3 px-5 py-2.5 text-sm font-body transition-colors duration-150 ${
                isActive
                  ? "bg-accent/10 text-accent"
                  : "text-muted-foreground hover:bg-secondary hover:text-foreground"
              }`}
            >
              {isActive && <span className="absolute bottom-1 left-0 top-1 w-0.5 bg-accent" />}
              {item.icon}
...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-background">
      <aside className="fixed left-0 top-0 z-30 hidden h-full w-56 flex-col border-r border-border bg-card md:flex">
        <NavContent {...navProps} />
      </aside>
      ...
      <AnimatePresence>
        {drawerOpen && (
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            />
            <motion.aside
              className="absolute left-0 top-0 h-full w-56 border-r border-border bg-card"
              variants={
                reduced
                  ? undefined
                  : {
